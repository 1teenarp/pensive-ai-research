...
echo; echo "[1] Memory controller ECC summary (ras-mc-ctl)"
echo "----------------------------------------------------------------------"
...
echo; echo "[2] Live per-channel Corrected/Uncorrectable counters (EDAC sysfs)"
echo "----------------------------------------------------------------------"
...
echo
echo "Cross-reference the CE-heavy channels found in [2] with the table above."
...
echo "For a warranty/RMA: attach [1]/[2] counters + the reset-reason history in [5]."

echo; echo "[5] Reset-reason history (uncorrectable escalation evidence)"
echo "----------------------------------------------------------------------"
grep -a "Previous system reset" /var/log/syslog /var/log/kern.log 2>/dev/null | tail -6
echo
echo "NOTE: A record of corrected->uncorrectable escalation (channel G = slot MM4 here) plus"
echo "      reset reason 0x08000a00 is strong evidence for a vendor RMA."
echo "======================================================================"
