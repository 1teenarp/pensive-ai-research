...
| **P12** | **Serving runs on official images + vendor-published weights only.** No third-party docker images, no third-party quantized checkpoints, no third-party draft/speculator models as a serving path. | User directive 2026-09-16; case: the GLM-5.3-Flash sm_120 block (R-014/R-015) is solved by a community vLLM fork (custom NoPE sparse-MLA kernels + EXL3 quant + research-licensed CC BY-NC-ND draft model) on 2×96 GB cards. We *read* it for what it proves, we do not serve from it. If such a project works where we are blocked, extract the method — read its source/config, port the approach into our own image, or file it upstream — and record the provenance in the recipe. |
